                                          ...
                                          ICloneable
        {
            ...
            public PersianDate(int year, int month, int day, int hour, int minute, int second)
            {
                try
                {
                    ...
                    {
                        throw new ArgumentException("Date time is not valid " + year.ToString());
            ...
            public static string GetNameMiladi(string EnglishName)
            {
                if (EnglishName.Substring(0, 1) == "0")
            ...
            public static DateTime GetMiladi(PersianDate DateValue)
            {
                return new DateTime(DateValue.Year, DateValue.Month, DateValue.Day, DateValue.Hour, DateValue.Minute, DateValue.Second, new PersianCalendar());
            ...
            public static PersianDate Parse(string dateString)
            {
                try
                {
                    DateTime dttime = DateTime.Parse(dateString);
                    return Parse(dttime);
                }
                catch { return PersianDate.MinValue; }
            }
            #endregion
